...
class KafkaChannelsManager(PubSubManager):  # pragma: no cover
    ...
    def __init__(self, url='kafka://localhost:9092', channel='socketio', write_only=False):
        ...
        super(KafkaChannelsManager, self).__init__(channel=channel,
    ...
    def _publish(self, data):
        ...
        try:
            _channel = data['data']['channel']
        except:
            pass
        logger.debug(f'use produce @channel: {_channel}')

        #self.producer.send(_channel, value=pickle.dumps(data).encode('utf-8'))
        self.producer.send(_channel, data)
        self.producer.flush()
